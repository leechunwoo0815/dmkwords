...
class QuizService:
    ...
    def quiz_status_batch(self, child: Child, book_ids: list[int]) -> list[dict]:
        ...
        finished_ids = {
            r[0]
            for r in self.db.query(ReadingProgress.book_id)
            .filter(
        ...
        passed_ids = set(
            r[0]
            for r in self.db.query(WordsLedger.book_id)
            .filter(
        ...
        total_map = {
            bid: (
                self.db.query(func.max(QuizAttempt.total_questions))
                .filter(
                    QuizAttempt.child_id == child.id,
