...
class ReservationService:
    ...
    def expire_due(self) -> int:
        ...
        for item in due:
            ...
            copy = (
                self.db.query(BookCopy).filter(BookCopy.id == res.copy_id).with_for_update().first()
            )
            if copy and copy.status == BookCopy.STATUS_RESERVED:  # 锁后当前读
                copy.status = BookCopy.STATUS_AVAILABLE
            event_bus.publish(
...
class ReservationAdminService:
    ...
    def checkout(self, admin, reservation_id: int):
        ...
        if _res_pre:
            self.db.query(ChildModel).filter(
                ChildModel.id == _res_pre.child_id
...
# VocabularyService / FavoriteService 拆出至 vocabulary_service.py（god file 800 行限制）；
# re-export 保持 miniapp_router / 测试引用路径不变
from backend.domain.reading.vocabulary_service import (  # noqa: E402, F401  # isort: skip
    FavoriteService,
    ShelfService,
    VocabularyService,
)
