...
CONFIG_SEEDS = [
    ...
    (
        ...
        "int",
        "借阅",
        "预约到期前提醒时点（距 expires_at 小时）",
        "预约到期前提醒（小时）",
    ),
    (
        "pending_evaluation_remind_days",
        "7",
        "int",
        "会员",
        "待评估超过 N 天每周提醒馆员",
    ...
    (
        "first_activity_90d_remind_days",
        "90",
        "int",
        "会员",
        "99 元首场活动购后提醒转年费（天）",
    ...
    (
        "withdrawal_review_timeout_days",
        "3",
        "int",
        "会员",
        "退会申请审核超时（天）自动取消",
    ...
    # 媒体体积（2026-09-17 用户裁定：服务器磁盘有限，上传图服务端必压、生成图控体积）
    (
        "image_upload_max_mb",
        "8",
        "int",
        "媒体",
        "单张上传图片体积上限（MB），超过直接拒绝（解码前拦截）",
        "上传图片体积上限（MB）",
    ),
    (
        "image_upload_max_output_kb",
        "600",
        "int",
        "媒体",
        "上传图压缩后的体积上限（KB），超标则逐档降质/缩边",
        "上传图输出上限（KB）",
    ),
    (
        "image_jpeg_quality",
    ...
    (
        "image_generated_jpeg_quality",
        "85",
        "int",
        "媒体",
        "生成图（阅读圈卡片/周报月报）JPEG 质量（1-95）",
        "生成图 JPEG 质量",
    ),
    (
        ...
        "int",
        "媒体",
        "图书封面长边上限（px）：列表 340rpx、详情 220rpx，1080 已足够清晰",
        "图书封面长边上限（px）",
    ),
    (
        "image_activity_cover_max_edge",
        "1200",
        "int",
        "媒体",
        "活动封面长边上限（px，横版 banner）",
        "活动封面长边上限（px）",
    ),
    (
        "image_doc_max_edge",
        ...
        "int",
        "媒体",
        "文档类图片长边上限（px）：收款凭证/观察报告，需放大看清小字",
        "凭证与报告长边上限（px）",
    ),
    # 媒体体检（docs/15 §二十二）：两条都是"防误删"的闸门，不是业务规则
    (
        "media_trash_min_age_minutes",
        "60",
        "int",
        "媒体",
        "清理孤儿图的最小年龄（分钟）：文件 mtime 距今不足此值则拒绝移入回收站"
        "（上传链路先写文件、后落库引用，此闸门防两步之间的竞态误伤新图）",
        "媒体清理最小年龄（分钟）",
    ),
    (
        "media_trash_retain_days",
        "30",
        "int",
        "媒体",
        "回收站保留天数：到期后由每日任务**复检引用**再清除；复检发现在用则自动还原",
        "媒体回收站保留天数（天）",
    ),
]
