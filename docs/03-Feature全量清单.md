...
| ID | 名称 | 价值 | 优先级 | 依赖 | 验收标准 | Gherkin | 状态 |
|---|---|---|---|---|---|---|---|
| FEAT-084 | 阅读圈（类朋友圈成就分享） | 增长激励 | P1 | F7,F10 | 小程序第 5 个 tabBar（首页/图书馆/书架/我的+阅读圈，微信 tabBar 上限 5）；卡片全部后端生成**零 UGC**（不触发微信内容安全审查），一期 MVP 6 类模板=A 类终身卡 5 种（里程碑〔数据源 MilestoneAward+milestone_nodes 配置〕/读本数〔累计 10/30/50/100 本，新节点体系可配置〕/等级〔A→B→C 晋级〕/满分〔测验 5/5 满分通过〕/连击〔连续打卡 7/30/100 天〕）+完读卡（读完《书名》+N 词入账——最高频需限频防刷）；双署名「家长显示名 · 孩子英文名」（孩子侧沿用 R-317/318：英文名/昵称+头像，无全名无手机号；家长显示名半公开默认直接用）；发布=**家长主动授权晒**（双入口：事件触发点「晒成就」按钮〔里程碑通知/完读引导弹窗/满分结果页〕→卡片预览→分享落帖 + 常驻入口〔阅读护照「我的成就」历史成就补晒+阅读圈页顶部〕）；信息流时间倒序真分页+下拉刷新，点卡全屏预览（wx.previewImage 支持长按保存——卡片带馆 branding，家长转发现实朋友圈=免费拉新），空态「还没有小伙伴晒成就——读完一本书，来做第一个！」；**仅点赞**（parent_id+post_id 唯一、可取消、显计数），明确不做：评论/用户传图/用户文字（微信审核红线）；被赞通知 scene: circle.liked（「张女士 赞了 Tommy 的成就」，馆长赞特殊文案，设置可关闭）；防滥用：同成就终身一晒（child_id+card_type+ref_id 唯一）+每日限晒 2 帖+帖子不可编辑（card_data JSON 快照冻结）+删除权仅家长与超管；管理端「阅读圈」模块：Tab1 帖子管理（列表含卡片缩略图/双署名/卡片类型/点赞数/馆长赞标记/发布时间；筛选类型下拉含「全部」+时间段+家长孩子关键词搜索〔客服定位〕；**未赞徽标**〔侧边栏+页顶胶囊「今日新帖 N 条待赞」〕；**行内一键馆长赞**〔巡场动线 30 秒〕；置顶同时最多 1 条〔小程序带「馆方推荐」角标〕；删除必填原因审计留痕）+Tab2 运营概览（本周新帖数/分享家长数/点赞总数/馆长赞覆盖率〔目标 100%〕/卡片类型分布〔反哺模板迭代〕）；小程序馆长赞特殊态（金色边框/徽标 🌟）；微信审核受阻降级预案：改「荣誉墙」模式（仅自己+馆方可见，点赞保留）；明确不做清单：举报流程（零 UGC 无违规内容面）/运营代发帖（数据必须真实）/评论管理/敏感词过滤；二期挂账 WM14-B：排名卡（上榜/上升——依赖每周一排行榜快照任务，APScheduler 已有基建）+周报卡+突破卡（单日词数创新高）+社区横幅+远期活动纪念卡；技术概要（供 WM14 开发参考）：新域 reading_circle，Post 表（child_id/parent_id/card_type/card_data JSON/image_path/like_count/admin_liked/is_pinned）+Like 表（post_id+parent_id 唯一），Pillow 卡片模板引擎复用 gen_cover 管线（新增卡片图片字段须枚举全部消费端逐一接 token——媒体消费点清单化纪律），API 约 5 个（GET /circle/posts 分页/POST /circle/posts 晒卡〔服务端校验成就归属+限频〕/POST+DELETE /{id}/like/GET /circle/my-cards 可晒成就库）+管理端帖子列表/馆长赞/置顶/删除端点；交付切分 WM14-A（一期 MVP 全量）/WM14-B（排名+周报+突破卡+横幅，依赖 A）/WM14-C（远期按需）〔PRD §7.5；D8 先例：@draft 草稿 behave.ini 已排除零门禁〕 | reading_circle.feature（@draft 六组场景） | IMPLEMENTED（WM14-A 一期 MVP 2026-09-10 交付，待用户验收） |
