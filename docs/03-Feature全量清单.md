...
| FEAT-083 | 会员购买/续费动线（R3 占位承接，插修 16 登记） | 会员 | P1 | 29 号插修16 | 小程序会员购买页（观察期/年费两档——微信支付 WM12 上线后接支付；本期占位跳转 member 页）。来源：用户 20260908 反馈"应该直接跳转让他买观察期会员或年费会员的界面，虽然现在还没有"。插修 16 R3 已做播放入口前置拦截+三态引导文案占位跳转（TODO 锚点 book-detail.js onPlay），界面上线后一行替换 | REGISTERED（排期待定，依赖 WM12 支付） |
| FEAT-082 | 活动封面+管理端详情编辑+小程序轮播/热门（S5 需求登记，插修 12） | 线下活动 | P2 | 22 号插修12 | 活动支持封面图（上传/存储走 R-316 同款通道 cover/{活动id}.jpg）；管理端活动详情查看+编辑（发布后可改未开始活动的 info/封面/名额，动资金字段需谨慎——fee 修改只影响新报名）；小程序首页轮播位（有封面活动优先）+热门活动（报名数排序）〔PRD §9 活动域增补草案——见 docs/03 附录 Gherkin 场景（D8 先例：behave 无步骤定义炸 gate，实现时移入 features/）〕 |IMPLEMENTED（第四批 T45 全链交付+插修 16/17 消费点收口+用户目视通过 2026-09-09） |
