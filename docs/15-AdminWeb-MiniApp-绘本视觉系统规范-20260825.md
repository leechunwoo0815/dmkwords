...
## 二十二、媒体体检与回收站（2026-09-23 立，用户裁定后）

> **来源**：用户 2026-09-23 裁定两阶段——① 每天自动"点数"孤儿图、**只看不删**；② 同时要**一个删除按钮**，硬条件是「**不会删掉正在使用的图片**」。
> 本节是这条硬条件的规范落地。**改这条链路前先读本节**（它管的是"删磁盘文件"，比样式事故严重一个量级）。

### 22.1 四个名词（先把话说清）

| 名词 | 定义 |
|---|---|
| **引用文件** | DB 里被任一业务列引用的 uploads 相对路径（**含软删行**——软删记录 purge 时才删文件，提前删会造成悬空） |
| **可再生目录**（白名单） | 内容可由程序重新生成：`cover/` `book_audio/` `circle/` `reports/` |
| **保护目录** | 人工上传、**不可再生**：`voucher/`（收款凭证）`observation/`（评估报告图）`activity_detail/`（活动图文配图）`posters/` `voice/`，以及 `*-samples/`、`miniapp-audit*` 证据目录 |
| **孤儿** | 不在引用集、不属保护目录、**且落在白名单内**的落盘文件 |

四个条件同时成立才叫孤儿。**白名单外的一律"不算孤儿"**——宁可漏报不可误删（与 `cleanup_uploads.py` 同保守口径）。

### 22.2 五道防线（"不会删掉正在使用的图"的机械保证）

| # | 防线 | 说明 |
|---|---|---|
| 1 | **引用集即时复算** | 「移到回收站」与「到期清除」两个动作都**在事务里重新查 DB**——不信任前端传参、不信任磁盘上昨天那份报告。口径单一来源 `backend/domain/admin/media_service.py: collect_referenced`（`scripts/cleanup_uploads.py` 同样 import 它；红线 51「同一口径只能有一份来源」） |
| 2 | **保护目录 + 白名单双重收窄** | 保护目录直接拒；不在白名单内直接拒（"不在名单里"默认安全） |
| 3 | **最小年龄**（`media_trash_min_age_minutes`，默认 60） | 文件 mtime 距今不足该分钟数 → 拒绝移入。防的是**竞态**：上传链路"先写文件、后落库引用"，若在两步之间触发清理就会误伤刚上传的新图 |
| 4 | **只移动、不物理删除** | 移到 `uploads/.trash/<批次时间戳>/<原相对路径>`：原路径消失但字节还在。`.trash/` **不进盘点**、**不被 `/api/admin/uploads/{path}` 下发**（404）、**不在任何清理白名单里** |
| 5 | **清除前再复检一次引用** | 回收站条目到期（`media_trash_retain_days`，默认 30 天）后由每日任务处理——复检发现它**又被引用了**（例：某本书重新上传了同一张图）→ **还原回原位**，不删除 |
| 6 | **「清空回收站」= 提前到期，不是绕过防线**（2026-09-23 用户追加） | 用户不想为回收站再等 30 天。该动作**逐条走与第 5 道完全相同**的复检：仍被引用 → 还原；无人引用 → 物理删除。它是把"30 天等待"压缩成"现在"，**五道防线一道不少**；界面必须写明不可恢复 + 原因必填（审计 `media.purge`） |
| 7 | **回收站记账对账**（2026-09-23 实修） | 磁盘事实（`uploads/.trash/`）与账（`media_trash_entries`）可能脱节——**演示库清场重建会清掉记账表**（当日实测：19:29 的清理搬走 3452 张，随后清场重建即"文件还在、账没了"→ 界面看不见、也不会被到期清除，成隐形库存）。每日盘点先跑 `reconcile_trash()`：**正被引用且 uploads 已无 → 搬回原位**；正被引用但 uploads 也有一份 → 只报不动；无人引用 → 补账（保留期重新起算）。**对账只补账/搬回，绝不删除** |
