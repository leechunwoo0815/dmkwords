...
> 版本：v1.5.2（2026-09-21：**新增「文档先行」强制铁律（甲方原话"必须写进最高宪法"）** + 借阅额度口径修订——逾期不再额外扣减、全端不拦截）
> 版本沿革：v1.5.1（2026-09-20：DoD 3b「验收通过必打 tag」条款——专家 Q14 裁定授权增补；门禁第 5/8 步口径更新）← v1.5（2026-09-15：门禁清单按实际 9 步重写 + 知识库索引死链修正 + 技术栈文件名更正）← v1.4（门禁后现场恢复入 DoD）← v1.3（2026-08-28，新增功能闭环必更文档纪律）
...
| **借阅并发** | 借书锁主体行（Child）串行化 + 副本行锁 + SQL 原子库存更新；**可借额度 = 30 − 在借总数（含逾期）− 预约占用**；同书同副本并发唯一。〔**2026-09-21 甲方口径修订**：原条文"可借上限 = 30 − 逾期未还数"作废——逾期**只占它自己那一个名额**，不再对上限做额外扣减；且**任何一端都不因逾期拦截借书/预约**（管理端借书、小程序预约一视同仁），逾期的作用只剩"催还名单 + 逾期书本身不可续借 + 逾期通知"〕 |
...
> 🔴 **前置铁律：文档先行（2026-09-21 甲方强制令，原话"我们以后必须要文档先行，再动代码！这是强制性要求，必须写进最高宪法"）**
> 任何**功能开发、缺陷修复、口径变更**，一律按此顺序：**① 先写/改文档**（PRD 或任务包 → 受影响的 docs/*、features/*.feature、口径表、验收手册）→ **② 文档落地后**才允许动代码 → ③ 跑门禁 → ④ 回写台账。
> - **判据（可机械检查）**：`git diff --name-only` 里若只有 `backend/**`、`admin-web/**`、`miniapp/**` 等代码文件而**没有任何文档改动**，则该轮违反本铁律——除非该轮是纯样式/纯文案微调，或文档已在同一批的前序提交里改过（须在简报中指名）。
> - **口径类变更（业务规则的数值/判定顺序/拦截面）必须先改 PRD 与相关文档并注明修订日期**，历史口径按"增量增补"体例保留、不回改。
> - **禁止**"先写代码后补文档"、"代码顺带改口径而不动文档"、"文档留着下次一起补"。文档不是交付的附件，是施工的前置图纸。
> - 违规记入错误记忆库，并在简报里明确写出"本轮违反文档先行"（诚实第一）。
...
- **D 级不跑门禁、不为它推送或等 CI（2026-09-20 用户明令）**：纯文档刀没有可执行断言，跑 9 步全量门禁和一轮 CI 是 8~16 分钟的纯浪费——文档修补攒够、合并进下一次代码推送；
  **唯一例外**：在 `docs/` / `error_list/` / `外部专家意见/` 下**新增未跟踪文件**会被门禁第 [9] 步交付完整性拦，stage 时一并带上即可（不必为此单独开门禁）。
