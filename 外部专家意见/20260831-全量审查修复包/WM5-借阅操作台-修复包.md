# WM5 借阅操作台修复包（20260831 全量审查）

> **★ 修复状态（2026-09-01 开发模型回写；hash 由 git log 现场实取）**
>
> - **WM5-F1 续借错行 ✅ DONE**（commit `adeacc3`）：render 补第二参数
>   `r: BorrowRecordResponse`，apiRenew(r.id) 替代 find() 全局查找；
>   A 级 tsc exit 0；**目视未做（如实声明）**——本卡为写操作，验收动线
>   WM5 续借场景目视复核（专家验收放行清单已含："借 2 本点第 2 行续借 → +7 天落第 2 行"）。
>   连带：circulation.ts 补导出 BorrowRecordResponse（首轮 tsc 引用链补齐）。
> - WM5-F2（还书弹窗不关）**未在本批范围**——验收时若撞到按编号报专家定性。
>
