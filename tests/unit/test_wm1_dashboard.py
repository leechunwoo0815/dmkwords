...
def test_dashboard_overview_fields(client: TestClient, admin_headers: dict) -> None:
    ...
    assert (
        body["config_count"] == 39
    )  # 29 基础键 + ar_warning_range（C16）+ book_onboarding_check（D1）+ 6 项 WM11 提醒配置（2026-08-29）+ 2 项 WM14-A 阅读圈配置（circle_daily_post_limit / circle_books_count_nodes，2026-09-10）
