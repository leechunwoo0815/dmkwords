...
def test_empty_dev_code_rejects_all(client: TestClient, monkeypatch):
    ...
    monkeypatch.setattr(get_settings.cache_clear.__wrapped__ if hasattr(get_settings, "cache_clear") else get_settings, "_x", None, raising=False) if False else None
    monkeypatch.setenv("LOGIN_DEV_CODE", "")  # BaseSettings 读 env：修复前字段不存在被忽略（1234 恒过 = RED）；修复后读到空串全拒
    get_settings.cache_clear()
